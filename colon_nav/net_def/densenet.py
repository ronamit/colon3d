...
class _DenseLayer(nn.Module):
    ...
    def any_requires_grad(self, inputs: list[Tensor]) -> bool:
        return any(tensor.requires_grad for tensor in inputs)
    ...
    @torch.jit._overload_method  # noqa: SLF001
    def forward(self, input: list[Tensor]) -> Tensor:  # noqa: A002
        pass

    @torch.jit._overload_method  # noqa: SLF001
    def forward(self, input: Tensor) -> Tensor:  # noqa: F811, A002
        pass

    # torchscript does not yet support *args, so we overload method
    # allowing it to take either a List[Tensor] or single Tensor
    def forward(self, inputs: Tensor) -> Tensor:  # noqa: F811
        prev_features = [inputs] if isinstance(inputs, Tensor) else inputs

        if self.memory_efficient and self.any_requires_grad(prev_features):
            if torch.jit.is_scripting():
                raise Exception("Memory Efficient not supported in JIT")  # noqa: TRY002
...
class DenseNet(nn.Module):
    ...
    Args:
        ...
        output_dim (int): Length of the output vector.
    ...
    def __init__(
        ...
        self.features = nn.Sequential(
            OrderedDict(
                [
                    ("conv0", nn.Conv2d(self.n_input_channels, num_init_features, kernel_size=7, stride=2, padding=3, bias=False)),
