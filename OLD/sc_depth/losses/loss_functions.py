...
from sc_depth.losses.inverse_warp import inverse_warp
from sc_depth.losses.mask_ranking_loss import Mask_Ranking_Loss
from sc_depth.losses.normal_ranking_loss import EdgeguidedNormalRankingLoss
...
def mean_on_mask(diff, valid_mask):
    ...
    mean_value = (diff * mask).sum() / mask.sum() if mask.sum() > 100 else torch.tensor(0).float().to(device)
    return mean_value
