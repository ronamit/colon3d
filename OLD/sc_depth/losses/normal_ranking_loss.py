...
def edgeGuidedSampling(inputs, targets, edges_img, thetas_img, masks, h, w):
    ...
    sample_num = minlen
    index_anchors = torch.randint(0, minlen, (sample_num,), dtype=torch.long).to(device)
...
class EdgeguidedNormalRankingLoss(nn.Module):
    ...
    def getEdge(self, images):
        n, c, h, w = images.size()
        a = torch.Tensor([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]]).to(device).view((1, 1, 3, 3)).repeat(1, 1, 1, 1)
        b = torch.Tensor([[1, 2, 1], [0, 0, 0], [-1, -2, -1]]).to(device).view((1, 1, 3, 3)).repeat(1, 1, 1, 1)
        if c == 3:
    ...
    def getNormalEdge(self, normals):
        ...
        a = torch.Tensor([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]]).to(device).view((1, 1, 3, 3)).repeat(3, 1, 1, 1)
        b = torch.Tensor([[1, 2, 1], [0, 0, 0], [-1, -2, -1]]).to(device).view((1, 1, 3, 3)).repeat(3, 1, 1, 1)
