...
class TestSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.dataset == "nyu":
            depth = torch.from_numpy(imread(self.depths[index]).astype(np.float32)).float() / 5000
        elif self.dataset == "kitti":
            depth = torch.from_numpy(np.load(self.depths[index]).astype(np.float32))
        elif self.dataset == "ddad":
            depth = torch.from_numpy(load_sparse_depth(self.depths[index]).astype(np.float32))
