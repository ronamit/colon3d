...
from colon3d.general_util import find_between_str, find_in_file_between_str, path_to_str, save_plot_and_close
...
def create_metadata(seq_in_path, seq_out_path, args):
    seq_id = get_seq_id_from_path(seq_in_path)
    sim_settings_path = seq_in_path / "MySettings.set"
    # copy the settings file to the dataset folder
    shutil.copy2(sim_settings_path, seq_out_path / "Sim_GUI_Settings.set")
    ...
    intrinsics_data_path = seq_in_path / (seq_id + "_Intrinsic Data.txt")
    shutil.copy2(intrinsics_data_path, seq_out_path / "intrinsic_data.txt")
    ...
    metadata_path = seq_out_path / file_name
    with metadata_path.open("w") as file:
        yaml.dump(metadata, file)

    # copy any screenshots of the simulator GUI, if exist
    for file_path in seq_in_path.glob("Screenshot*.png"):
        shutil.copy2(file_path, seq_out_path / file_path.name)
    return metadata, n_frames


# --------------------------------------------------------------------------------------------------------------------
def save_rgb_video(seq_in_path: Path, seq_out_path: Path, vid_file_name: str, fps: float, n_frames: int):
    output_path = seq_out_path / (vid_file_name + ".mp4")
    seq_id = get_seq_id_from_path(seq_in_path)
    frames_paths = list(seq_in_path.glob(f"{seq_id}_*.png"))
    frames_paths.sort()
    assert (
        len(frames_paths) >= n_frames
    ), f"Only {len(frames_paths)} frames were found in {seq_in_path}, but {n_frames} are required"
...
def load_camera_motion(seq_in_path: Path, n_frames: int):
...
def save_ground_truth_depth_and_cam_poses(
    ...
    seq_id = get_seq_id_from_path(seq_in_path)
    depth_files_paths = list(seq_in_path.glob(f"{seq_id}_depth*.exr"))
    ...
    cam_poses = load_camera_motion(seq_in_path, n_frames)

    # save as h5 file
    output_path = seq_out_path / "gt_depth_and_cam_poses.h5"
    with h5py.File(output_path, "w") as hf:
        hf.create_dataset("z_depth_map", data=z_depth_frames, compression="gzip")
        hf.create_dataset("cam_poses", data=cam_poses)

    assert z_depth_frames.shape[0] == cam_poses.shape[0], "Number of depth frames and camera poses should be the same."
    # print histogram
    print(f"Z-Depth frames saved to: {output_path}.h5")
    plt.hist(z_depth_frames.flatten(), bins="auto")
    save_plot_and_close(seq_out_path / "z_depth_histogram.png")
    ...
    with (seq_out_path / "depth_info.pkl").open("wb") as file:
