...
class DetectionsTracker:
    ...
    def __init__(self, example_path, video_loader: VideoLoader):
        ...
        if not detections_file_path.is_file():
            ...
            self.detections_original = pd.read_csv(detections_file_path)
            self.detections_original = self.detections_original.astype({"frame_idx": int, "track_id": int})
    ...
    def get_tracks_in_frame(self, frame_idx: int, frame_type: str = "alg_view"):
        ...
        if frame_type == "alg_view":
            detections = self.detections_for_alg
        elif frame_type == "full_view":
            detections = self.detections_original
        else:
    ...
    def truncate_detections_for_alg_view(self):
        ...
        n_detects_orig = len(self.detections_original)
        new_xmin = np.zeros(n_detects_orig)
        new_xmax = np.zeros(n_detects_orig)
        new_ymin = np.zeros(n_detects_orig)
        new_ymax = np.zeros(n_detects_orig)
        is_valid = np.zeros(n_detects_orig, dtype=bool)
        for i, cur_detect in self.detections_original.iterrows():
            # get the o pixel coordinates inside the original detection box in the full frame
            x_coords = np.arange(round(cur_detect.xmin), round(cur_detect.xmax) + 1)
            y_coords = np.arange(round(cur_detect.ymin), round(cur_detect.ymax) + 1)
            ...
            if n_pix_in_in_alg_view <= n_pix_in_orig_box * 0.5:
                # too few pixels in the detection box are in the alg-view FOV circle
                # so we determine the detection box is not in the alg-view
                continue
            is_valid[i] = True
            # get the new detection box coordinates in the cropped area
        ...
        detections_new = pd.DataFrame(
            {
                "frame_idx": self.detections_original["frame_idx"][is_valid],
                "track_id": self.detections_original["track_id"][is_valid],
        ...
        self.detections_for_alg = detections_new
