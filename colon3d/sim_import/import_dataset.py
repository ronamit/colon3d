...
from colon3d.sim_import import load_ColonNav, load_SimCol3D, load_Zhang22
...
def main():
    ...
    parser.add_argument(
        "--raw_sim_data_path",
        type=str,
        default="data_gcp/raw_datasets/ColonNav",  #  ColonNav | Zhang22| SimCol3D |
        help="The path to the folder with the raw simulated scenes to load (generated by Unity).",
    )
    parser.add_argument(
        "--path_to_save_data",
        type=str,
        default="data/datasets/SimCol3D",  #  ColonNav/TrainData | ColonNav/TestData | SimCol3D/train | SimCol3D/test
    ...
    parser.add_argument(
        ...
        choices=["Train", "Test"],
        default="test",
        help="The name of the split to load (Train or Test). relevant for ColonNav and SimCold3d datasets.",
    )
    parser.add_argument(
        "--limit_n_scenes",
        type=int,
    ...
    def run(self):
        ...
        for i_scene in range(n_scenes):
            ...
            if self.sim_name in ["ColonNav", "SimCol3D"]:
                ...
                if self.sim_name == "ColonNav":
                    z_depth_frames = load_ColonNav.get_ground_truth_depth(
                        input_data_path=self.raw_sim_data_path,
                        depth_frames_paths=depth_frames_paths_per_scene[i_scene],
                        metadata=metadata,
                    )
                else:
                    z_depth_frames = load_SimCol3D.get_ground_truth_depth(
                        input_data_path=self.raw_sim_data_path,
                        depth_frames_paths=depth_frames_paths_per_scene[i_scene],
                        metadata=metadata,
                    )
