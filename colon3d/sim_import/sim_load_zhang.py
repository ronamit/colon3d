...
def load_sim_raw(
    ...
    scenes_names = [Path(p.name) for p in input_data_path.glob("Case*") if p.is_dir()]
    scenes_names.sort()
    if limit_n_scenes > 0:
        scenes_names = scenes_names[:limit_n_scenes]
    ...
    for scene_name in scenes_names:
        ...
        rgb_frames_paths = list((input_data_path / scene_name / cam_to_load).glob("*.png"))
        rgb_frames_paths = [Path(scene_name) / cam_to_load / p.name for p in rgb_frames_paths]
        rgb_frames_paths.sort()
        if limit_n_frames > 0:
            rgb_frames_paths = rgb_frames_paths[:limit_n_frames]
        assert len(rgb_frames_paths) > 0, f"no RGB frames found in {scene_name}"
