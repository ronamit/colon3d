...
from sc_depth.datasets.test_folder import TestSet
from sc_depth.losses.loss_functions import compute_errors
...
def main():
    ...
    print(f"{len(test_dataset)} samples found in test scenes")
    ...
    all_errs = []
    for _i, (tgt_img, gt_depth) in enumerate(tqdm(test_loader)):
        pred_depth = model(tgt_img.cuda())

        errs = compute_errors(gt_depth.cuda(), pred_depth, hparams.dataset_name)
    ...
    print(
        "\n  "
        + ("{:>8} | " * 9).format("abs_diff", "abs_rel", "sq_rel", "log10", "rmse", "rmse_log", "a1", "a2", "a3"),
    )
