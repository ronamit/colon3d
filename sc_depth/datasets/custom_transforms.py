        ...
        for im in images:
            if im.ndim < 3:  # depth
                scaled_images.append(
                    cv2.resize(im, dsize=(scaled_w, scaled_h), fx=1.0, fy=1.0, interpolation=cv2.INTER_NEAREST),
                )
            else:
    ...
    def __call__(self, images, intrinsics):
        ...
        if in_h != out_h or in_w != out_w:
            ...
            for im in images:
                if im.ndim < 3:  # depth
                    scaled_images.append(
                        cv2.resize(im, dsize=(out_w, out_h), fx=1.0, fy=1.0, interpolation=cv2.INTER_NEAREST),
                    )
                else:
